...
import argparse, tempfile, os, shutil, pickle, sys, re, glob, json, subprocess, multiprocessing as mp
import parsegbk, makeseqsql, makehmmsql, seqsql2fa, extractdbgenes, getrnagenes, setlog
import ete3methods, rangerdtl


print("\n \n ################# ARTS ################################## ARTS #################")
print("\n \n este arquivo esta sendo executado: artspipeline1.py")
...
def runantismash(aspath,infile,outdir,mcpu):
    ...
    if os.path.exists(aspath) and os.path.exists(infile):
        cmd = ["python",aspath,infile,"--outputfolder",asdir,"-v","--statusfile",asdir+"/statusfile.txt","--cpus",str(mcpu)]
        with open(asdir+"/aslog.txt",'w') as aslog:
            subprocess.call(cmd,stderr=aslog,stdout=aslog)
...
#reformat core table and write to file
...
def startquery(infile=None,refdir=None,td=None,rd=None,hmmdbs=None,rnahmm=None,cut=None,
               ...
               custcorehmms=False,custhmms=False,aspath=None,options="phyl,kres,duf",custorgname=None,prebuilttrees=False):
    try:
    ...
    try:
        ...
        if not os.path.isfile(infile):
            ...
            raise IOError
        try:
            queryorg, querygenus, clusters = parsegbk.convertgenes(infile,tdir,plasmid=True,userecnum=True,clust=True,rename=oldfname)
            if custorgname:
    ...
    try:
        ...
        if knownhmms and os.path.exists(knownhmms) and "kres" in options.lower():
            ...
            knownhits = {"data":[],"seqs":{}}
            print("\n ############## ARTS artspipeline1.py ########### \n")
            print("knownhits vazio line 624: \n", knownhits)
            print("\n ############## ARTS artspipeline1.py ########### \n")
            with open(os.path.join(tdir,"knownhits.domhr"),"r") as fil, open(os.path.join(tdir,"tables","knownhits.json"),"w") as ofil, open(os.path.join(tdir,"tables","knownhits.tsv"),"w") as tfil:
                ...
                for line in fil:
                    if not line.startswith("#"):
                        ...
                        seqid = x[0].split("|")[-1]
                        print("\n ############## ARTS artspipeline1.py ########### \n")
                        print("seqid line 636 antes de line 638: \n", seqid)
                        print("\n ############## ARTS artspipeline1.py ########### \n")
                        if seqid not in knownhits["seqs"]:
                            ...
                            print("x[-1] line 640: \n", x[-1])
                            print("\n ############## ARTS artspipeline1.py ########### \n")
                            print("x[seqid] line 640: \n", seqid)
                            print("\n ############## ARTS artspipeline1.py ########### \n")
                            print("knownhits teoricamente cheio line 640: \n", knownhits)
                            print("\n ############## ARTS artspipeline1.py ########### \n")
